...
---@class BUFFocus.Health.CenterText: BUFFontString
local centerTextHandler = {
	configPath = "unitFrames.focus.healthBar.centerText",
}
...
centerTextHandler.optionsTable = {
	...
	order = BUFFocusHealth.topGroupOrder.CENTER_TEXT,
	args = {},
}

ns.BUFFontString:ApplyMixin(centerTextHandler)

BUFFocusHealth.centerTextHandler = centerTextHandler
...
ns.dbDefaults.profile.unitFrames.focus.healthBar.centerText = {
	anchorPoint = "CENTER",
...
ns.options.args.focus.args.healthBar.args.centerText = centerTextHandler.optionsTable

function centerTextHandler:RefreshConfig()
	if not self.initialized then
		BUFFocus.FrameInit(self)

		self.fontString = BUFFocus.healthBarContainer.HealthBarText
