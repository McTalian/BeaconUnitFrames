...
---@class BUFFocus.Power.CenterText: BUFFontString
local centerTextHandler = {
	configPath = "unitFrames.focus.powerBar.centerText",
}
...
centerTextHandler.optionsTable = {
	...
	order = BUFFocusPower.topGroupOrder.CENTER_TEXT,
	args = {},
}

ns.BUFFontString:ApplyMixin(centerTextHandler)

BUFFocusPower.centerTextHandler = centerTextHandler
...
ns.dbDefaults.profile.unitFrames.focus.powerBar.centerText = {
	anchorPoint = "CENTER",
...
ns.options.args.focus.args.powerBar.args.centerText = centerTextHandler.optionsTable

function centerTextHandler:RefreshConfig()
	if not self.initialized then
		BUFFocus.FrameInit(self)

		self.fontString = BUFFocus.manaBar.ManaBarText
