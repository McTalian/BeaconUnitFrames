...
---@class BUFToFocus.Power.CenterText: BUFFontString
local centerTextHandler = {
	configPath = "unitFrames.tofocus.powerBar.centerText",
}
...
centerTextHandler.optionsTable = {
	...
	order = BUFToFocusPower.topGroupOrder.CENTER_TEXT,
	args = {},
}

ns.BUFFontString:ApplyMixin(centerTextHandler)

BUFToFocusPower.centerTextHandler = centerTextHandler
...
ns.dbDefaults.profile.unitFrames.tofocus.powerBar.centerText = {
	anchorPoint = "CENTER",
...
ns.options.args.tofocus.args.powerBar.args.centerText = centerTextHandler.optionsTable

function centerTextHandler:RefreshConfig()
	if not self.initialized then
		BUFToFocus.FrameInit(self)

		self.fontString = BUFToFocus.manaBar.ManaBarText
